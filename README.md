...
1. **自然對話框解析 (Conversational UI & LLM Routing)**：
   - 捨棄死板的過濾按鈕。使用者只需在直覺的文字對話框輸入如：「我們兩個人約會，錢不是問題，想吃午餐和溜冰」，後端 AI 就會像真人導遊一樣，自動推斷出四大核心要素（地點、種類、預算、人數），若有遺漏還會主動反問。
...
6. **無縫雙向即時翻譯 (Two-way Translation)**：
   - 打破語言隔閡！當外國旅客開啟系統時，不管是前台的對話框輸入，還是後端爬蟲擷取的在地店家評論與選單，AI 大腦都能依靠即刻的**雙向翻譯**建立溝通橋樑。外國人能使用母語發問，系統也能自動將最道地的台灣小吃名稱翻譯回饋，徹底實現無國界旅遊規劃！
