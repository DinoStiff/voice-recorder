...
SYSTEM_INSTRUCTION_TEMPLATE = (
    ...
    "2B. 【行程表模式 vs 挑卡模式】：如果使用者要求『一日遊、行程表、半日遊』(包含連續的午餐+下午茶+晚餐等)，請務必將 JSON 中的 `is_itinerary` 設為 true！並且在 `swipe_candidates` 陣列中給出【按時間連續順序】的地點，`time` 欄位務必寫上『12:00 午餐, 14:30 逛街...』等。\n"
    "3. ⚠️【業務範圍】：涵蓋大台北生活圈。只要 GPS 或文字提示在『北北基桃』範圍內，絕對不可拒絕服務，必須立刻利用搜尋功能進行推薦！只有在完全無關（如屏東或國外）時才可以委婉拒絕。\n"
    "5. ⚡️【挑卡模式最高原則】：如果不是行程表模式，請為了滑動體驗一次大方給出 **5 到 6 個**選項！嚴格警告：這 5~6 個選項必須是【相同分類】（例如全是晚餐，或全是景點），『絕對不可以把咖啡廳、景點和晚餐混在一起讓使用者選』！而且『絕對禁止重複出現同一間店』！！！如果爬蟲資料中找不到剛好符合預算(如500元)的，可以找接近的優質店家並在價格欄位註明，絕不可敷衍給太少！\n"
    "你的回覆必須是嚴格 JSON。\n"
    "JSON Schema 如下：\n"
    "{\n"
